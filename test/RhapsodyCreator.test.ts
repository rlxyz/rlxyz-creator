...
describe('RhapsodyCreator', () => {
  ...
  beforeEach(async () => {
    ...
    const RhapsodyCreator = await hre.ethers.getContractFactory('RhapsodyCreatorTest', deployer, overrides);
  ...
  describe('deployment', () => {
    it('should have an allocation promotion amount', async () => {
      expect(await creator.amountForPromotion()).to.equal(creatorTestParams.amountForPromotion);
    });

    it('should have set the correct max public mint per address', async () => {
      expect(await creator.maxPublicBatchPerAddress()).to.equal(creatorTestParams.maxPublicBatchPerAddress);
    });

    it('should have a valid mint price', async () => {
      expect(await creator.mintPrice()).to.equal(creatorTestParams.mintPrice);
    });
  });
  ...
  describe('tokenURI', () => {
    ...
    it('should return valid token uri', async () => {
      await creator.connect(minterA).publicMint(1, { value: parseEther(0.08) });
    ...
    it('should return valid token uri', async () => {
      await creator.connect(minterA).publicMint(5, { value: parseEther(0.08 * 5) });
      expect(await creator.tokenURI(0)).to.equal(baseURI + '0');
      expect(await creator.tokenURI(1)).to.equal(baseURI + '1');
      expect(await creator.tokenURI(2)).to.equal(baseURI + '2');
      expect(await creator.tokenURI(3)).to.equal(baseURI + '3');
  ...
  describe('sale', () => {
    describe('presaleMint', () => {
      ...
      beforeEach(async () => {
        ...
        await creatorA.setMintTime(currentBlockTime + 105, currentBlockTime + 110);

        presaleMinter = async (minter, invocations, maxInvocation, proof, ether) =>
          creatorA.connect(minter).presaleMint(invocations, maxInvocation, proof, {
      ...
      it('should be able to mint if address whitelisted', async () => {
        ...
        await expect(presaleMinter(minterA, 5, 5, proof, 0.08 * 5))
      ...
      it('should be able to mint if less than max invocation limit', async () => {
        let leaf = generateLeaf(minterA.address, 5);
        let proof = merklized.tree.getHexProof(leaf);
        await expect(presaleMinter(minterA, 4, 5, proof, 0.08 * 4))
          .to.emit(creatorA, 'Created')
          .withArgs(minterA.address, 4);

        leaf = generateLeaf(minterC.address, 5);
        proof = merklized.tree.getHexProof(leaf);
        await expect(presaleMinter(minterC, 1, 5, proof, 0.08))
          .to.emit(creatorA, 'Created')
      ...
      it('should fail if minting invocation is 0', async () => {
        let leaf = generateLeaf(minterA.address, 5);
        let proof = merklized.tree.getHexProof(leaf);
        await expect(presaleMinter(minterA, 0, 5, proof, 0)).to.be.revertedWith(
      ...
      it('should fail if trying to minting more than maxPresaleBatchPerAddress', async () => {
        let leaf = generateLeaf(minterA.address, 5);
        let proof = merklized.tree.getHexProof(leaf);
        await expect(presaleMinter(minterA, 6, 5, proof, 6 * 0.08)).to.be.revertedWith(
      ...
      it('should fail address proof if passed in invalid maxInvocations', async () => {
        let leaf = generateLeaf(minterA.address, 5);
        let proof = merklized.tree.getHexProof(leaf);
        await expect(presaleMinter(minterA, 1, 10, proof, 0.08)).to.be.revertedWith(
          'RhapsodyCreator/invalid-address-proof'
        );

        leaf = generateLeaf(minterB.address, 5);
        proof = merklized.tree.getHexProof(leaf);
        await expect(presaleMinter(minterB, 5, 10, proof, 0.08 * 5)).to.be.revertedWith(
          'RhapsodyCreator/invalid-address-proof'
        );
      });
      ...
      it('should fail if too much or too little eth supplied', async () => {
        let leaf = generateLeaf(minterA.address, 5);
        let proof = merklized.tree.getHexProof(leaf);
        await expect(presaleMinter(minterA, 1, 5, proof, 1.5)).to.be.revertedWith('RhapsodyCreator/invalid-mint-value');

        leaf = generateLeaf(minterB.address, 5);
        proof = merklized.tree.getHexProof(leaf);
        await expect(presaleMinter(minterB, 5, 10, proof, 0)).to.be.revertedWith('RhapsodyCreator/invalid-mint-value');
      });
      ...
      it('should only be able to mint once', async () => {
        ...
        await expect(presaleMinter(minterA, 4, 5, proof, 4 * 0.08))
          .to.emit(creatorA, 'Created')
          .withArgs(minterA.address, 4);

        await expect(presaleMinter(minterA, 1, 5, proof, 0.08)).to.to.be.revertedWith(
        ...
        await expect(presaleMinter(minterB, 4, 5, proof, 4 * 0.08))
          .to.emit(creatorA, 'Created')
          .withArgs(minterB.address, 4);

        await expect(presaleMinter(minterB, 2, 5, proof, 2 * 0.08)).to.be.revertedWith(
          'RhapsodyCreator/invalid-invocation-upper-boundary'
        );
        ...
        await expect(presaleMinter(minterC, 5, 5, proof, 5 * 0.08))
          .to.emit(creatorA, 'Created')
          .withArgs(minterC.address, 5);

        await expect(presaleMinter(minterC, 1, 5, proof, 1 * 0.08)).to.be.revertedWith(
          'RhapsodyCreator/invalid-invocation-upper-boundary'
        );
      });
      ...
      it('should not be able to transfer NFTs out and mint again', async () => {
        ...
        await expect(presaleMinter(minterA, 2, 5, proof, 2 * 0.08))
          .to.emit(creatorA, 'Created')
          .withArgs(minterA.address, 2);

        await creatorA.connect(minterA).transferFrom(minterA.address, minterB.address, 0);

        await creatorA.connect(minterA).transferFrom(minterA.address, minterB.address, 1);
      ...
      it('should not be able to mint if not whitelisted', async () => {
        ...
        await expect(presaleMinter(minterB, 1, 5, wrongProof, 1 * 0.08)).to.be.revertedWith(
          'RhapsodyCreator/invalid-address-proof'
        );
      });
      ...
      describe('variable whitelist', () => {
        ...
        it('should be able to variable mint', async () => {
          ...
          await expect(presaleMinter(minterA, 2, 4, proof, 2 * 0.08))
            .to.emit(creatorA, 'Created')
            .withArgs(minterA.address, 2);
          ...
          proof = merklizedA.tree.getHexProof(leaf);
          await expect(presaleMinter(minterC, 1, 3, proof, 1 * 0.08))
            .to.emit(creatorA, 'Created')
            .withArgs(minterC.address, 1);
        });
        ...
        it('should only allow max allocated variable amount in merkle root', async () => {
          let leaf = generateLeaf(minterA.address, 4);
          let proof = merklizedA.tree.getHexProof(leaf);
          await expect(presaleMinter(minterA, 4, 4, proof, 4 * 0.08))
            .to.emit(creatorA, 'Created')
            .withArgs(minterA.address, 4);

          leaf = generateLeaf(minterB.address, 5);
          proof = merklizedA.tree.getHexProof(leaf);
          await expect(presaleMinter(minterB, 5, 5, proof, 5 * 0.08))
            .to.emit(creatorA, 'Created')
        ...
        it('should fail if trying to mint more than max limit of an allocated address limit', async () => {
          let leaf = generateLeaf(minterA.address, 4);
          let proof = merklizedA.tree.getHexProof(leaf);
          await expect(presaleMinter(minterA, 5, 4, proof, 5 * 0.08)).to.be.revertedWith(
            'RhapsodyCreator/invalid-invocation-upper-boundary'
          );

          leaf = generateLeaf(minterC.address, 3);
          proof = merklizedA.tree.getHexProof(leaf);
          await expect(presaleMinter(minterC, 4, 3, proof, 4 * 0.08)).to.be.revertedWith(
            'RhapsodyCreator/invalid-invocation-upper-boundary'
          );
        });
      });
      ...
      describe('small collection', () => {
        ...
        beforeEach(async () => {
          ...
          creatorB = await RhapsodyCreator.deploy(12, 4, 0, creatorTestParams.mintPrice);
          await creatorB.setMintMerkleRoot(merklized.root);
          await creatorB.setMintTime(currentBlockTime + 105, currentBlockTime + 110);
        });
    ...
    describe('publicMint', () => {
      let publicMinter;
      beforeEach(async () => {
        await creator.setMintTime(currentBlockTime + 105, currentBlockTime + 110);
      ...
      it('should be able to mint nfts in public mint', async () => {
        await expect(publicMinter(minterA, 1, 0.08)).to.emit(creator, 'Created').withArgs(minterA.address, 1);

        await expect(publicMinter(minterA, 5, 5 * 0.08))
          .to.emit(creator, 'Created')
          .withArgs(minterA.address, 5);

        await expect(publicMinter(minterA, 14, 14 * 0.08))
          .to.emit(creator, 'Created')
          .withArgs(minterA.address, 14);

        await expect(publicMinter(minterB, 20, 0.08 * 20))
          .to.emit(creator, 'Created')
          .withArgs(minterB.address, 20);
      });

      it('should only be able to max batch amount of nfts', async () => {
        await expect(publicMinter(minterA, 20, 0.08 * 20))
          .to.emit(creator, 'Created')
          .withArgs(minterA.address, 20);

        await expect(publicMinter(minterB, 21, 0.08 * 21)).to.be.revertedWith(
          'RhapsodyCreator/invalid-invocation-upper-boundary'
        );
      });

      it('should only be able to mint max batch amount of nfts even in two or more txs', async () => {
        await expect(publicMinter(minterA, 5, 0.08 * 5))
          .to.emit(creator, 'Created')
          .withArgs(minterA.address, 5);

        await expect(publicMinter(minterA, 16, 0.08 * 16)).to.be.revertedWith(
          'RhapsodyCreator/invalid-invocation-upper-boundary'
        );
      });

      it('should fail if minting invocation is 0', async () => {
        await expect(publicMinter(minterA, 0, 0.08)).to.be.revertedWith('RhapsodyCreator/invalid-mint-value');
      });
    });
    ...
    describe('setMintTime', () => {
      ...
      it('can change presale and public time retroactively', async () => {
        ...
        // forward time
        await creator.setMintTime(currentBlockTime + 110, currentBlockTime + 115);
        ...
        // testing edge case
        await creator.setMintTime(currentBlockTime + 1, currentBlockTime + 2);
        expect(await creator.presaleTime()).to.be.equal(currentBlockTime + 1);
        expect(await creator.publicTime()).to.be.equal(currentBlockTime + 2);
      });

      it('public sale time cannot be less than presale time', async () => {
        await expect(creator.setMintTime(currentBlockTime + 110, currentBlockTime + 105)).to.be.revertedWith(
          'RhapsodyCreator/invalid-public-time'
        );

        // edge cases
        await expect(creator.setMintTime(currentBlockTime + 2, currentBlockTime + 1)).to.be.revertedWith(
          'RhapsodyCreator/invalid-public-time'
        );

        await expect(creator.setMintTime(currentBlockTime + 1, currentBlockTime + 0)).to.be.revertedWith(
          'RhapsodyCreator/invalid-public-time'
        );
      });

      it('presale time cannot be less than the current block timestamp', async () => {
        await expect(creator.setMintTime(currentBlockTime - 10, currentBlockTime + 10)).to.be.revertedWith(
          'RhapsodyCreator/invalid-presale-time'
        );

        await expect(creator.setMintTime(currentBlockTime - 1, currentBlockTime)).to.be.revertedWith(
          'RhapsodyCreator/invalid-presale-time'
        );
  ...
  describe('dev', () => {
    ...
    describe('promotionMint', () => {
      it('should be able to batch mint according to maxBatchSize', async () => {
        await expect(creator.promotionMint(100)).to.emit(creator, 'Created').withArgs(admin.address, 100);
      });

      it('should be able to batch mint according to maxBatchSize multiple times', async () => {
        await expect(creator.promotionMint(20)).to.emit(creator, 'Created').withArgs(admin.address, 20);
        await expect(creator.promotionMint(40)).to.emit(creator, 'Created').withArgs(admin.address, 40);
        await expect(creator.promotionMint(40)).to.emit(creator, 'Created').withArgs(admin.address, 40);
      });
      ...
      it('should only be able to mint max promotional nfts', async () => {
        ...
        await expect(creator.promotionMint(1)).to.be.revertedWith('RhapsodyCreator/invalid-promotion-supply');
      });

      it('should fail if not minting according to maxBatchSize', async () => {
        await expect(creator.promotionMint(1)).to.be.revertedWith('RhapsodyCreator/invalid-batch-multiple');
      });
      ...
      describe('totalSupply restrictions', () => {
        ...
        beforeEach(async () => {
          ...
          const RhapsodyCreator = await hre.ethers.getContractFactory('RhapsodyCreatorTest', admin, overrides);
          creatorA = await RhapsodyCreator.deploy(
            collectionSize,
        ...
        it('should not allow to mint more than promotionMint if some nfts already minted', async () => {
          await expect(creatorA.promotionMint(4)).to.emit(creatorA, 'Created').withArgs(admin.address, 4);

          await creatorA.setMintTime(currentBlockTime + 1, currentBlockTime + 2);

          await creatorA.connect(minterA).publicMint(2, { value: parseEther(0.08 * 2) });

          await expect(creatorA.promotionMint(4)).to.be.revertedWith('RhapsodyCreator/invalid-promotion-supply');
        });

        it('should only allow correct allocation of promotion mint even if late', async () => {
          await creatorA.setMintTime(currentBlockTime + 1, currentBlockTime + 2);
          await creatorA.connect(minterA).publicMint(2, { value: parseEther(0.08 * 2) });
          await expect(creatorA.promotionMint(2)).to.emit(creatorA, 'Created').withArgs(admin.address, 2);
          await expect(creatorA.promotionMint(2)).to.be.revertedWith('RhapsodyCreator/invalid-promotion-supply');
        });
      });
    });
  });
